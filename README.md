...
> **運用フロー・失敗時の挙動・トラブルシュート（"動いてる?"の調べ方・回収手順）は [`docs/RADAR.md`](docs/RADAR.md) を参照。**
...
`score >= threshold` のハイライト（最大 `deepdive_max` 件）は、記事本文を取得して
**全文翻訳＋構造化分析**を生成し、Notion の該当ページ本文に書き込みます。

- 📖 要約 / 📝 全文翻訳 / 🎯 立場として押さえるべきポイント / ⚠️ 批判的視点
- 本文取得: RSS 全文があればそれ、無ければ記事 URL から `trafilatura` で抽出。
  取得できない記事（JS依存・paywall 等）は要約のみにフォールバック（「⚠️ 全文取得不可」を明記）。
- Slack の `📝 Notion で開く` から、この深掘りページに直接飛べます。
- コスト・時間防御として 1 実行あたり `deepdive_max`（デフォルト 8）件まで。
- `medium-notion radar --no-deepdive` で深掘りを無効化（軽量動作）。dry-run でも深掘りしません。

> 深掘りは Claude 呼び出しが記事ごとに発生するため、毎朝の実行が数分長くなります。
