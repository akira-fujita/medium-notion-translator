...
5. 既存インデックス読み込み（1回）+ Notion DB の URL 取得（1回）。**スキップ判定は Notion DB のみを source of truth** とし、ローカル index は translator のコンテキスト（topics サジェスト・既存記事一覧）として渡す
6. ブラウザ初期化（1回 — 全記事で共有）
7. 記事ごとのループ:
   - 処理済みチェック（Notion DB に居るか） → スキップ or 取得 → 翻訳 → Notion 登録 → メモリ上でインデックス追加
...
**bookmark コマンドの処理フロー**:
1. 設定読み込み（1回）
2. 既存記事と照合（**Notion DB を source of truth**。ローカル index は translator コンテキスト用）
3. ブラウザ初期化
4. リストページへの遷移:
...
| Step 2 | メタデータ抽出 | 記事先頭 3000 文字 + 既存記事一覧 + 既存 Topics | JSON |
