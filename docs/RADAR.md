...
| 本文が取れない（RSS全文なし / 抽出失敗）| 翻訳を省き要約のみで **通常通り保存**（失敗ではない）|
| 深掘りが一時エラー（接続断 / 空応答 / 分析JSON不正）| **最大3回リトライ（2秒→4秒）**。回復すれば通常保存 |
| 深掘りがリトライ後も失敗 / タイムアウト | **その記事は書き込みも既読化もスキップ → 翌日再挑戦**。さらに**その run の残りの深掘りも打ち切り**（全断時の無駄呼び出し防止）|
| Claude CLI が見つからない（設定エラー）| 握りつぶさず **run を失敗させる**（静かに空登録しない）|
...
**重要な不変条件**: 「Notion に保存できた記事だけ既読になる」。だから障害が起きても記事は基本的に失われず、次回の run で拾い直される（※後述の限界あり — フィードから外れた記事は対象外）。
...
- **深掘り失敗の再挑戦は保証ではない**: 未読判定は「今回取得したフィード項目」に対してのみ行う。記事がフィードから外れる／`DEFAULT_FEED_LIMIT` を超えると拾えない。翌朝もフィードに残っている前提の best-effort
- **深掘り失敗の再挑戦は再分類に非対応**: 失敗記事が翌日 `others` 落ち／`deepdive_max` 超えになると、深掘りなしで保存され得る（永続 pending 管理は未実装）
- Notion 作成と既読化は非アトミック。Notion 成功後に既読化が落ちると翌日重複し得る（`append_item` の URL 冪等化は未対応）
